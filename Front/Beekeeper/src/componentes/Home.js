...
const Home = ({ setViewState }) => {
    return (
        <div className="gestion-container">
            <header className="perfil-header " style={{textAlign:"left"}}>
                <div>
            ...
            <div className="stats-grid-dashboard animate-fade-in" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '30px', marginTop: '20px' }}>
                <div 
                    className="stat-card highlight clickable-card" 
                    onClick={() => setViewState('VerMisApiarios')}
                    style={{ padding: '40px', minHeight: '220px', display: 'flex', flexDirection: 'column', justifyContent: 'center', cursor: 'pointer' }}
                >
                    <label>Entorno</label>
                    <h2>Apiarios</h2>
                ...
                <div 
                    ...
                    onClick={() => setViewState('VerMisColmenas')}
                    style={{ padding: '40px', minHeight: '220px', display: 'flex', flexDirection: 'column', justifyContent: 'center', cursor: 'pointer' }}
                >
                ...
                <div 
                    className="stat-card clickable-card" 
                    onClick={() => setViewState('VerAlimentacion')}
                    style={{ padding: '40px', minHeight: '220px', display: 'flex', flexDirection: 'column', justifyContent: 'center', cursor: 'pointer' }}
                >
                ...
                <div 
                    className="stat-card highlight clickable-card" 
                    onClick={() => setViewState('VerCosechas')}
                    style={{ padding: '40px', minHeight: '220px', display: 'flex', flexDirection: 'column', justifyContent: 'center', cursor: 'pointer' }}
                >
                ...
                <div 
                    className="stat-card highlight clickable-card" 
                    onClick={() => setViewState('Estadisticas')}
                    style={{ gridColumn: 'span 2', padding: '50px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', minHeight: '200px', justifyContent: 'center', cursor: 'pointer' }}
                >
                    <label>Inteligencia</label>
                    <h2>Análisis</h2>
                    <p style={{ maxWidth: '800px', fontSize: '1.1rem' }}>Visualiza el rendimiento basado en la altitud y pisos térmicos. Optimiza tus traslados con datos reales de eficiencia.</p>
                </div>
            </div>
